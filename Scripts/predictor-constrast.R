...
condition_search_str <- str_split(condition_names[1], regex("\\.[lmh]{1}_")) |> 
  map(\(pred) 
      str_detect(pred, paste("female", collapse = "|")) |>
        ifelse(
          pred |>
            str_split_i("\\.", 1) |>
            paste0(... = _, ".[lmh]"),
...
playdat <- playdat_full |>
  ...
  mutate(pred1 = condition |> str_extract(paste0("female", "\\.[lmh]{1}")) |> str_split_i("\\.", 2),
         .keep = "unused",
         .before = everything()) |>
  ...
  mutate(woa = pmap_dbl(list(pc1, pc2, pc3, pc4, a, b), \(pc1, pc2, pc3, pc4, a, b) {
    data.frame(res = rmultinom(1, 1, c(pc1, pc2, pc3, pc4)),
      ...
      pull(woa)})) |>
  select(rep, pred1, b) |>
  mutate(gender = recode_values(pred1,
                                "-1 SD" ~ "male",
                                "+1 SD" ~ "female"),
         .keep = "unused") |>
  pivot_wider(names_from = gender, values_from = b) |>
