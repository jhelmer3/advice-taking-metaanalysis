...
fit <- qs2::qs_read(here::here("Models", "tri-m_more-pred_model_2.1.qs"))
...
rhats <- fit$metadata()$stan_variables |> 
  map(\(var) tibble(param = var,
                    rhat = fit$draws(variables = var) |>
                      posterior::rhat()),
      .progress = T) |>
  list_rbind()
...
# draws |> names() |> str_replace("\\[\\d+,*\\d*\\]", "") |> unique()

mcmc_rank_overlay(fit$draws(), c("Intercept_eta1", "Intercept_eta2", "Intercept_eta3"))
mcmc_trace(fit$draws(), c("Intercept_eta1", "Intercept_eta2", "Intercept_eta3"))
mcmc_rank_overlay(fit$draws(), c("mu", "phi"))

mcmc_rank_overlay(fit$draws(), 
                  map(sample.int(16454, size = 16), 
                      \(i) glue::glue("r_3_eta2_1[{i}]")) |> 
                    list_c())

mcmc_rank_overlay(fit$draws(), 
                  map(sample.int(16454, size = 16), 
                      \(i) glue::glue("r_8_eta3_1[{i}]")) |> 
                    list_c())

mcmc_rank_overlay(fit$draws(),
                  draws |> select(matches("sd_2")) |> names())
