data {
  ...
  array[N] int Y1;  // categorical response variable for hurdle (D = 2, A = 3, or C = 1?) (midpoint 4)
...
parameters {
  ...
  array[M_1] vector[N_1] z_5;  // standardized group-level effects person-level for m hurdle
  vector<lower = 0>[M_2] sd_6;  // group-level standard deviations item-level for m hurdle
  array[M_2] vector[N_2] z_6;  // standardized group-level effects item-level for m hurdle
...
model {
  ...
  for (n in 1:N) {
    ...
    phi[n] += r_1_1b[J_1[n]] * Z_1_1[n] + r_2_1b[J_2[n]] * Z_2_1[n];
...
generated quantities {
  ...
  for (n in 1:N) {
    ...
    // add more terms to the linear predictor
    mu = Intercept + Xc[n] * b + r_1_1[J_1[n]] * Z_1_1[n] + r_2_1[J_2[n]] * Z_2_1[n];
    phi = Intercept_phi + Xc[n] * b_dist + r_1_1b[J_1[n]] * Z_1_1[n] + r_2_1b[J_2[n]] * Z_2_1[n];
